...
SERVICE_CAPS = sdk_ci_demopy
SPEC_FILE = sdk_ci_demopy.spec
URL = https://kbase.us/services/sdk_ci_demopy
DIR = $(shell pwd)
LIB_DIR = lib
SCRIPTS_DIR = scripts
TEST_DIR = test
...
build-test-script:
	...
	echo 'python -m nose --with-coverage --cover-package=$(SERVICE_CAPS) --cover-xml --cover-xml-file=/kb/module/work/test_coverage/coverage.xml --nocapture  --nologcapture .' >> $(TEST_DIR)/$(TEST_SCRIPT_NAME)
